...
public class MapGenerator : MonoBehaviour
{
        ...
        for (int i = 0; i < 17; i++)
        {
            ...
            if (i % 2 == 0)
            {
                for (int j = 0; j < 9; j++)
                {
                    ...
                    points[i][j * 2] = new Point(Instantiate(platform, spawnPosition, Quaternion.identity),j * 2, i);
                    ...
                    {
                        spawnPosition.x += 0.3f;
                        points[i][j * 2 + 1] = new Point(Instantiate(wall, spawnPosition, Quaternion.Euler(0f, 0f, 90f)),j * 2 + 1  ,i);
            ...
            else
            {
                ...
                {
                    spawnPosition.x += 0.3f;
                    points[i][j * 2] = new Point(Instantiate(wall, spawnPosition, Quaternion.identity),j * 2 , i);
                    points[i][j * 2].GameObject.GetComponent<CoordinateScript>().Y = i;
                    points[i][j * 2].GameObject.GetComponent<CoordinateScript>().X = j * 2;
    ...
    private void SpawnPlayers()
    {
        ...
        Player1 = new Player(Instantiate(player, positionPlayer1, Quaternion.identity), 16, 0, 8, 16, 9, "Player 1");
        ...
        Player2 = new Player(Instantiate(player, positionPlayer2, Quaternion.identity), 0, 16, 8, 4, 9, "Player 2");
